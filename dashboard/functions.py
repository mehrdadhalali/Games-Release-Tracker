...
def display_game_table(show_nsfw, os_selection, start_date, end_date, search_query, sort_by):
    ...
    table_data['Price'] = table_data['Price'].apply(lambda x: f"£{x:.2f}")  # Format prices
    ...
    if sort_by == "Price (Ascending)":
        ...
        table_data = table_data.sort_values("Price", key=lambda x: x.str.replace('£', '').replace('Free', '0').astype(float), ascending=False)
